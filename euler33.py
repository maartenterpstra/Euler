...
for i in range(10, limit):
	...
	for j in range(i + 1, limit):
		...
		if len(inter) > 0:
			# Sets dont do indexing. These sets contain only one element so popping yields that element
			...
			if new_j > 0 and Decimal(i) / Decimal(j) == Decimal(new_i) / Decimal(new_j):
				fracs.append(Fraction(i,j))

# Result is denominator of reduced fraction. The Fraction class does reduction automatically
